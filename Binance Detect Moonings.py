...
session_profit = 0
global session_profit_USDT
session_profit_USDT = 0.0
...
def wait_for_price():
    ...
    print(f'not enough time has passed yet...Session profit:{session_profit:.2f}% Session Profit USDT: {session_profit_USDT:.2f}$')
    ...
    for excoin in externals:
        ...
        for test in tickers:
            if test == excoin[:-4]:
                valid_coin=True
        if valid_coin == False : print (f'\033[91mnok : {excoin}')
        if valid_coin and (excoin not in volatile_coins and excoin not in coins_bought and (len(coins_bought) + exnumber) < MAX_COINS):
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        if float(last_price[coin]['price']) < SL or (float(last_price[coin]['price']) > TP and not USE_TRAILING_STOP_LOSS):
            print(f"{txcolors.SELL_PROFIT if PriceChange >= 0. else txcolors.SELL_LOSS}TP or SL reached, selling {coins_bought[coin]['volume']} {coin} - {BuyPrice} - {LastPrice} : {PriceChange:.2f}% Amount in USD {(float(coins_bought[coin]['volume']*(LastPrice-BuyPrice))):.3f} {txcolors.DEFAULT}")
            #print(f"{txcolors.SELL_PROFIT if PriceChange >= 0. else txcolors.SELL_LOSS}Amount in USD {(float(coins_bought[coin]['volume']*(LastPrice-BuyPrice))):.3f}")
            ...
            # run the else block if coin has been sold and create a dict for each coin sold
                ...
                if LOG_TRADES:
                    ...
                    session_profit_USDT=session_profit_USDT + profit
            continue
